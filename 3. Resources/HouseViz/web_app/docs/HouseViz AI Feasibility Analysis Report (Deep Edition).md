...
> 摘要结论（TL;DR）  
> HouseViz.ai 以 **3D 日照可视化为切入点**、进而通过 **AI 报告 + AI 视频** 成为房产经纪人的营销工具与 C 端选房辅助，商业可行性高。关键成功要素是：**低成本快速 MVP（Maptiler + Maplibre GL JS + Next.js + Three.js + suncalc）→ 经纪人渠道分发（降低 CAC）→ 逐步引入户型图解析与高精度数据以做差异化壁垒**。需注意的数据许可、法律合规与 AI 输出可追溯性风险，但都有明确的缓解路径。
...
- 前端：Next.js（React 同构）、Maplibre GL JS（2D 地图）、Three.js（3D 可视化）、WebWorker（几何计算）。
...
- 首选：OpenStreetMap（footprint）+ Maptiler 矢量瓦片（渲染）。
...
- 覆盖率监测（运行时）：
    - `coverage_footprint = 有 footprint 的建筑数量 / 视口内建筑总数`。
    
    - `coverage_height_levels = 有 height 或 levels 的建筑数量 / 视口内建筑总数`。
    
    - 当 `coverage_height_levels < 阈值（建议 0.5–0.7）` 时，UI 显示“估算精度受限”提示。
        
- 回退链路（从上到下）：
    - Maptiler 矢量瓦片（building/height/levels）。
        
    - OSM Overpass（补拉 footprint 与 building:levels）。
        
    - 手动绘制 footprint（内置 draw 工具）。
        
    - 上传平面/红线图（可选，后续对齐）→ 暂存为项目私有数据。
        
- 高度/层数缺失时的估算：
    - 默认层高 `3.0m`（住宅）/ `3.5m`（商业），支持用户切换；
        
    - `estimatedHeight = totalFloors × floorHeight`，并在报告中标注“估算”。
        
- 本地与服务端缓存：
    - 视口 footprint/属性缓存 TTL（建议 7 天）；
        
    - 用户覆盖（楼层、层高）与来源版本一起保存，便于可追溯；
        
    - 对热门区域预热附近建筑数据，降低首屏延迟。
        
- 城市白/黑名单与降级：
    - 首批上线城市需 `coverage_height_levels ≥ 0.7`；
        
    - 不达标城市默认降级为 2D 阴影动画与估算报告，明确提示精度限制。
        
- 限流与重试：
    - Overpass 请求加速率限制与指数退避；
        
    - 超时回退到最近缓存或提示用户稍后重试。
        
- 数据更新：
    - 记录 `source` 与 `fetched_at`；按城市批量刷新（周维度），使用 ETag/If-None-Match 避免重复流量。
...
- **地图/数据许可**：遵循 OSM ODbL 要求（署名/共享），Maptiler TOS。任何外部数据采购签署合法授权合同。
...
- 直接可变成本（每单/每用户）：
    - Maptiler/地图调用与带宽：$0.001–$0.01/次（视缓存与用量分级）。
        
    - 报告生成（PDF 存储与传输）：$0.005–$0.05/份。
        
    - 视频生成（第三方 API/自建渲染）：$2–$8/条（按时长/质量）。
        
- 固定/半固定成本：
    - 云主机与队列、日志、监控：$1k–$4k/月（MVP 阶段）。
        
    - 人力（独立开发 + 兼职设计/后端）：$5k–$15k/月（弹性）。
...
- 核心指标：
    ...
    - LTV/CAC ≥ 3 为健康阈值；回本周期 ≤ 6 个月更优。
        
- 示例（B2B 订阅 $29/月）：
    - 假设月度可变成本/经纪人 ≈ $2（地图/报告/视频少量使用）。
        
    - 毛利率 ≈ (29 − 2) / 29 ≈ 93%。
        
    - 若 CAC = $60，经常性留存 12 个月：LTV ≈ 29 × 12 × 93% ≈ $323；LTV/CAC ≈ 5.4。
...
- Viral Loop（可追踪）：
    - 报告/视频分享落地页带推荐码，注册/付费绑定归因；
        
    - “协作查看”邀请（同公司经纪人自动归属同一团队）。
        
- 激励：
    - 经纪人邀请经纪人：每邀请 1 名付费用户奖励 1 个月订阅或 10 份额外报告额度；
        
    - 新用户首单报告 $1（拉新）；高价值功能（视频生成）试用券。
...
- 内容增长：
    - “30 秒看懂采光”短视频模板组件，周更 3 条，自动发布到 TikTok/YouTube/小红书；
        
    - 城市/楼盘案例库（SEO 结构化数据 + schema 标注）。
...
- 关键事件：
    - map_view_loaded（含城市、缩放级别、瓦片命中率）。
        
    - building_clicked（含是否有 height/levels、覆盖来源）。
        
    - floor_confirmed、simulate_opened、time_scrubbed（滑动次数/停留时长）。
        
    - report_generated、report_downloaded、video_generated、share_clicked。
        
    - signup_completed、trial_started、subscribe_started/paid、invite_sent/accepted。
        
- 指标分层：
    - 城市/渠道/版本 分层看转化与留存；
        
    - 漏斗：地图加载 → 建筑点击 → 楼层确认 → 模拟打开 → 导出报告 → 注册/付费。
        
- 工具建议：
    - 产品分析：PostHog/Amplitude；
        
    - 数据仓库：BigQuery/Redshift（冷数据）；
        
    - 可视化：Metabase/Superset；
        
    - 报警：渲染失败率、生成耗时 P95/99、覆盖率跌破阈值。
...
- **独立开发者 + 外包策略**（lean）
    ...
    - 你（独立开发）负责核心工程（Next.js + Maptiler + Maplibre + Three.js）。
...
## 附录 A：城市优先名单与数据覆盖评估模板

> 用于筛选首批上线与营销试点的城市列表与量化打分
...
- 估计活跃经纪人数（千）
- 房价中位数（美金）
- 对日照关注度（低/中/高，基于本地文化/法规/搜索热度）
- 竞争者存在度（低/中/高）
- 合作渠道可得性（协会/经纪公司/媒体）评分（1–5）
- 综合评分（加权）：`0.25*覆盖 + 0.25*可用率 + 0.2*经纪人数 + 0.15*关注度 + 0.15*渠道`

筛选规则：

- 覆盖率与可用率均 ≥ 70%；
- 综合评分 Top 10 入选首批 MVP 城市；
- 日本/韩国优先同分倾斜。